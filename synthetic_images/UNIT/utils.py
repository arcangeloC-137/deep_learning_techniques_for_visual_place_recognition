...
def load_vgg16(model_dir):
    ...
    if not os.path.exists(os.path.join(model_dir, 'vgg16.weight')):
        if not os.path.exists(os.path.join(model_dir, 'vgg16.py')):
            os.system('wget https://www.dropbox.com/s/76l3rt4kyi3s8x7/vgg16.py?dl=1 -O ' + os.path.join(model_dir, 'vgg16.py'))
        vgglua = torch.load(os.path.join(model_dir, 'vgg16.py'))
        vgg = Vgg16()
