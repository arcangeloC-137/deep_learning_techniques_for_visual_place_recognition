...
base_transform = transforms.Compose([
    transforms.ToTensor(),
    # mean and std values computed from Imagenet
...
class BaseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img = transforms.functional.adjust_brightness(img: img, brightness_factor: self.night_brt_factor)
...
class TripletsDataset(BaseDataset):
    ...
    def night_data_augmentation(self, img, brightness_factor=.55, solarize_threshold=.25):
        # solarize_threshold must be a number between [0,1]
        img_solr = TF.solarize(img, solarize_threshold)
        # brightness_factor must be a number between [0,1]
        img_solr_bright = TF.adjust_brightness(img_solr, brightness_factor)
        return img_solr_bright

    def perspective_change_fill(self, img, distortion_scale=.5, change_probability=.7, fill_value=1):

        img_different_perspective = transforms.RandomPerspective(distortion_scale, p=change_probability, fill=fill_value).forward(img)

        return img_different_perspective

    def perspective_change_crop(self, img, distortion_scale=.5, change_probability=.7, height=300, width=400):

        img_different_perspective = transforms.CenterCrop((height, width)).forward(transforms.RandomPerspective(distortion_scale, p=change_probability, fill=0).forward(img))
    ...
    def __getitem__(self, index):
        ...
        query = None
        transformed = False
        # Data Augmentation by user check
        if(self.args.night_data_augmentation):
            
            img_q = None
            if(query is None):
                img_q = path_to_pil_img(self.queries_paths[query_index])
            else:
                img_q = query
            
            if(self.args.adjust_brightness != None and self.args.solarize != None):
                query = self.night_data_augmentation(img_q, self.args.adjust_brightness, self.args.solarize)
            else:
                query = self.night_data_augmentation(img_q)
            
        
        if(self.args.perspective_fill_data_augmentation):
            ...
            if(self.args.distortion_scale != None and self.args.perspective_prob != None and self.args.perspective_fill!= None):
                query = self.perspective_change_fill(img_q, self.args.distortion_scale, self.args.perspective_prob, self.args.perspective_fill)
            else:
                query = self.perspective_change_fill(img_q)


        if(self.args.perspective_crop_data_augmentation):
            ...
            if(self.args.distortion_scale != None and self.args.perspective_prob != None and self.args.height!= None and self.args.width!= None):
                query = self.perspective_change_crop(img_q, self.args.distortion_scale, self.args.perspective_prob, self.args.height, self.args.width)
            else:
                query = self.perspective_change_crop(img_q)


        if(self.args.occlusion_data_augmentation):
            ...
            query = self.query_transform(img_q)
            transformed = True

            if(self.args.perspective_prob != None):
                query = self.add_occlusions(query, self.args.perspective_prob)
            else:
                query = self.add_occlusions(query)
