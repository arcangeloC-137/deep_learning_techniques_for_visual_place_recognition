...
class modelA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.backbone1 = get_backbone1(args)
        self.arch_name = args.backbone1
...
class modelB(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.backbone2 = get_backbone2(args)
        self.arch_name = args.backbone2
        self.aggregation = get_aggregation(args)
        self.self_att = False
...
class MyEnsemble(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.modelB(x)
        v = torch.add(x1, x2)
        #x = torch.div(v, 2) second approach for ensembling
        print(x.shape)
        return x
...
def get_backbone1(args):
    args.work_with_tokens = args.backbone1.startswith('cct') or args.backbone1.startswith('vit')
    if args.backbone1.startswith("alexnet"):
        backbone1 = torchvision.models.alexnet(pretrained=True)
        layers = list(backbone1.features.children())[:-2]
        for l in layers[:5]:
            for p in l.parameters(): p.requires_grad = False
        logging.debug("Train last layers of the alexnet, freeze the previous ones")
    backbone1 = torch.nn.Sequential(*layers)
    args.features_dim = get_output_channels_dim(backbone1)  # Dinamically obtain number of channels in output
    return backbone1
##########################################################################################################################
def get_backbone2(args):
    args.work_with_tokens = args.backbone2.startswith('cct') or args.backbone2.startswith('vit')
    if args.backbone2.startswith("resnet"):
    ...
    backbone2 = torch.nn.Sequential(*layers)
    args.features_dim = get_output_channels_dim(backbone2)  # Dinamically obtain number of channels in output
    return backbone2
##########################################################################################################################
