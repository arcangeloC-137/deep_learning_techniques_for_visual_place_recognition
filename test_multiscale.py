    ...
    with torch.no_grad():
        ...
        if args.multiscale:
            for inputs, indices in tqdm(database_dataloader, ncols=100):
                ...
                #print(features.size())
                step_vector = args.step_multiscale
                for step in step_vector:
                    input_resized = inputs[:,:,::step,::step] #leave list and channel untouched, reduce width and height
                    features_resized = model.module.backbone(input_resized)
                    features_resized = features_resized.view(features_resized.size(0),features_resized.size(1),-1)
                    #print(features_resized.size())
                    if args.multiscale_method == 'concat':                
                        features = torch.cat([features, features_resized], dim=2)
                    elif args.multiscale_method == 'average':
                        features = features + features_resized.repeat(1,1,step**2)[:,:,:1200]
                    else:
                        print('Error: parameter multiscale_method can be concat or average')

                if args.multiscale_method == 'average':
                    features = features/(len(step_vector)+1)
			  
                features = features.unsqueeze(3)
                vlad = model.module.aggregation(features)
                vlad = vlad.detach().cpu().numpy()

                if pca != None:
                    vlad = pca.transform(vlad)
                all_features [indices.detach().numpy(), :] = vlad
                del inputs, features, vlad
        
        #if multiscale is not required... leave as the original file
        else: 
          for inputs, indices in tqdm(database_dataloader, ncols=100):
              features = model(inputs.to(args.device))
        ...
        if args.multiscale:
            for inputs, indices in tqdm(queries_dataloader, ncols=100):
                ...
                step_vector = args.step_multiscale
                for step in step_vector:
                    input_resized = inputs[:,:,::step,::step]
                    features_resized = model.module.backbone(input_resized)
                    features_resized = features_resized.view(features_resized.size(0),features_resized.size(1),-1)
                ...
                vlad = model.module.aggregation(features)
                if test_method == "five_crops":  # Compute mean along the 5 crops
                    vlad = torch.stack(torch.split(vlad, 5)).mean(1)
                vlad = vlad.detach().cpu().numpy()
